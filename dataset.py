...
class ModelNet40(Dataset):
    def __init__(self, dataset_path: str, mode: str = 'test', categories: List = None):
        ...
        with open(os.path.join(dataset_path, '{}_files.txt'.format(mode))) as fid:
    ...
    def __getitem__(self, item):
        points, label = self._data[item, :, :], self._labels[item]
        return torch.Tensor(points), label
...
class ShapeNetCompletion(Dataset):
    def __init__(self, root, npoints=2500,
                 split='train', class_choice=None, normal_channel=False):
        self.npoints = npoints
        self.root = root
    ...
    def __getitem__(self, index):
        viewpoints = [torch.Tensor([1, 0, 0]), torch.Tensor([0, 0, 1]), torch.Tensor([1, 0, 1]),
                      torch.Tensor([-1, 0, 0])]
        viewpoint = viewpoints[index % 4]
        ...
        crop_num = point_set.shape[0] // 4
        points = torch.Tensor(point_set)
        crop_idx = torch.topk(((points - viewpoint.unsqueeze(0)) ** 2).sum(1), dim=0, k=crop_num, largest=False)[1]
        remain_idx = torch.topk(((points - viewpoint.unsqueeze(0)) ** 2).sum(1), dim=0, k=points.shape[0]-crop_num, largest=True)[1]
        crop_cloud, remain_cloud = points[crop_idx], points[remain_idx]
        return remain_cloud, points, cls

    def __len__(self):
        return len(self.datapath) * 4


if __name__ == '__main__':
    modelnet40_dataset = ModelNet40(dataset_path="D:/dataset/modelnet40_ply_hdf5_2048", mode="train")
    print(len(modelnet40_dataset))
    x = modelnet40_dataset[0]